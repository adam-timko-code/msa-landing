...
const features = [
  { icon: <Shield className="h-6 w-6" aria-hidden />, title: "Verified Profiles", desc: "ID + selfie match to reduce catfish and throwaway accounts." },
  { icon: <IdCard className="h-6 w-6" aria-hidden />, title: "Smart ID Check", desc: "On-device checks with liveness—privacy-first by design." },
  { icon: <Smartphone className="h-6 w-6" aria-hidden />, title: "One-Tap SOS", desc: "Silent alert with live location to trusted contacts." },
  { icon: <MapPin className="h-6 w-6" aria-hidden />, title: "Meet Plan", desc: "Share where/when you’ll meet. Auto-expires when you’re safe." },
  { icon: <BellRing className="h-6 w-6" aria-hidden />, title: "Auto Check-ins", desc: "Light nudges during the meet; escalate only if needed." },
  { icon: <HeartHandshake className="h-6 w-6" aria-hidden />, title: "Trusted Community", desc: "Build reputation with verified, safe meetups." },
];

const tiers = [
  { name: "Starter", price: "$0", period: "/mo", bullets: ["Verify 1 profile", "1 trusted contact", "Basic meet plan", "Email support"], cta: "Get started", highlight: false },
  { name: "Plus", price: "$4", period: "/mo", bullets: ["Passkey sign-in", "Up to 5 contacts", "Auto check-ins", "Priority support"], cta: "Go Plus", highlight: true },
  { name: "Pro", price: "$8", period: "/mo", bullets: ["Unlimited contacts", "Export meet receipts", "Advanced controls", "Early access"], cta: "Go Pro", highlight: false },
];
  ...
  >
      <header className="sticky top-0 z-40 backdrop-blur-md bg-black/30 border-b border-white/10">
        <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-2xl bg-indigo-600 text-white grid place-items-center font-bold">MSA</div>
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" className="hidden sm:inline-flex text-gray-200 hover:text-white">Sign in</Button>
            <Button className="rounded-2xl bg-white/10 text-white hover:bg-white/20">Get the App</Button>
          </div>
        </div>
      </header>
      ...
      <section className="container mx-auto px-6 py-20 md:py-28">
        <div className="grid md:grid-cols-1 gap-10 items-center">
          <div>
            ...
            <p className="mt-5 text-lg text-gray-100/90 max-w-prose">
            ...
            <div className="mt-6 flex items-center gap-4 text-sm text-gray-100/80">
      ...
      <section id="how" className="mx-auto max-w-6xl px-6 pb-8">
        <div className="text-center mb-10">
          ...
          <p className="mt-3 text-gray-100/85">Three simple steps to safer meets.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <Card key={n} className="rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 text-gray-100">
              <CardHeader><CardTitle className="text-xl">Step {n}</CardTitle></CardHeader>
              <CardContent className="text-gray-200">
                {n === 1 && <p>Verify each other with ID + selfie and pick what to share. No public posting, no stalking surface.</p>}
                {n === 2 && <p>Create a meet plan with time window, venue, and discreet check-ins. Add trusted contacts in one tap.</p>}
      ...
      <section id="features" className="mx-auto max-w-6xl px-6 py-8">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-white">Built for real-world safety</h2>
          <p className="mt-3 text-gray-100/85">Privacy-first design and practical tools that actually get used.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <Card key={i} className="rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 text-gray-100">
              <CardHeader className="pb-2"><CardTitle className="text-lg flex items-center gap-2">{f.icon}{f.title}</CardTitle></CardHeader>
      ...
      <section className="mx-auto max-w-4xl px-6 py-10">
        <Card className="rounded-3xl bg-white/10 backdrop-blur-md border border-white/15 text-gray-100">
          <CardContent className="p-6 md:p-10">
            <div className="md:flex items-center justify-between gap-8">
              <div className="max-w-xl">
                <h3 className="text-2xl font-bold text-white">Be first to the beta</h3>
                <p className="mt-2 text-gray-200">Join the early access list. We’ll only email you about launch and major updates.</p>
              </div>
              <form className="mt-6 md:mt-0 flex w-full md:w-auto gap-3" onSubmit={(e)=>e.preventDefault()}>
                <Input type="email" placeholder="you@example.com" className="rounded-2xl min-w-[240px] bg-white/85 text-gray-900 placeholder:text-gray-600" required />
                <Button type="submit" className="rounded-2xl bg-white/10 text-white hover:bg-white/20">Notify me</Button>
              </form>
            </div>
          </CardContent>
        </Card>
      ...
      <section id="pricing" className="mx-auto max-w-6xl px-6 py-10">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-white">Simple, transparent pricing</h2>
          <p className="mt-3 text-gray-100/85">Start free. Upgrade when you want more control.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {tiers.map((t, i) => (
            <Card key={i} className={`rounded-2xl bg-white/10 backdrop-blur-md border text-gray-100 ${t.highlight ? 'border-indigo-300 shadow-xl' : 'border-white/15'}`}>
              <CardHeader>
                <CardTitle className="text-xl flex items-center justify-between">
                  {t.name}
                  {t.highlight && <span className="text-xs px-2 py-1 rounded-full bg-indigo-600 text-white">Most popular</span>}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-end gap-1">
                  <span className="text-4xl font-extrabold text-white">{t.price}</span>
                  <span className="text-gray-200 mb-1">{t.period}</span>
                </div>
                ...
                <Button className="w-full mt-6 rounded-2xl bg-white/10 text-white hover:bg-white/20">{t.cta}</Button>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <section id="faq" className="mx-auto max-w-5xl px-6 py-12">
        ...
        <div className="grid md:grid-cols-2 gap-6 text-gray-200">
          <div>
            ...
            <p className="mt-2">No. Location pings only during an active meet window or SOS, and you control who sees it.</p>
          </div>
          <div>
            <h3 className="font-semibold text-white">How does verification work?</h3>
            <p className="mt-2">We combine ID scan, selfie liveness, and optional signals. You choose what to share and with whom.</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-2">We nudge you first. If there’s no response, we escalate to your trusted contacts per your rules.</p>
          </div>
        </div>
      </section>

      <footer className="border-top border-white/10 bg-black/30 backdrop-blur-md">
        <div className="mx-auto max-w-6xl px-6 py-10 grid md:grid-cols-3 gap-6 text-sm text-gray-300">
          <div>
            <div className="h-8 w-8 rounded-xl bg-indigo-600 text-white grid place-items-center font-bold">MSA</div>
          ...
          <div className="space-y-2">
            ...
            <a href="#" className="block hover:text-white">iOS (TestFlight)</a>
            <a href="#" className="block hover:text-white">Android (Beta)</a>
            <a href="#" className="block hover:text-white">Web demo</a>
          </div>
        </div>
      </footer>
