...
const features = [
  ...
  },
  {
    ...
    title: "One-Tap SOS",
    desc: "Silent alert with location to your trusted contacts.",
  },
  {
  ...
  },
  {
    ...
    desc: "Lightweight nudges during the meet; escalate only if needed.",
  },
  {
    icon: <HeartHandshake className="h-6 w-6" aria-hidden />,
    title: "Trusted Community",
    desc: "Build a reputation score with verified, safe meetups.",
  },
];
...
export default function App() {
  ...
  async function startVerifyQR() {
    try {
      setBusy(true);
      // Try server — fall back locally so a QR always shows
      let j = null;
      try {
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="mt-5 text-lg text-slate-700 max-w-prose">
              <em>Identity verified. Plans shared. Safety built in.</em> Connect confidently with privacy-first tools that turn first meets into safer meets.
            </p>
  ...
  >
    Verify Me (QR)
            ...
            <div className="rounded-3xl bg-white shadow-xl p-4 md:p-6 border border-slate-200">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <Card className="rounded-2xl">
                  ...
                  <CardContent className="text-slate-600">ID + selfie match builds trust without oversharing.</CardContent>
                </Card>
                <Card className="rounded-2xl">
                  <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><MapPin className="h-4 w-4" /> Plan</CardTitle></CardHeader>
                  <CardContent className="text-slate-600">Send a safety brief: where, when, and who you’re meeting.</CardContent>
                </Card>
                <Card className="rounded-2xl">
                  <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><BellRing className="h-4 w-4" /> Check-in</CardTitle></CardHeader>
                  <CardContent className="text-slate-600">Automatic check-ins with quick replies or safe words.</CardContent>
                </Card>
                <Card className="rounded-2xl">
                  <CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Smartphone className="h-4 w-4" /> SOS</CardTitle></CardHeader>
                  <CardContent className="text-slate-600">Silent SOS shares live location and calls your people.</CardContent>
                </Card>
              </div>
            </div>
          </motion.div>
      ...
      <section id="pricing" className="mx-auto max-w-6xl px-6 py-16">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {tiers.map((t, i) => (
            <Card key={i} className={`rounded-2xl border ${t.highlight ? 'border-brand-600 shadow-xl' : 'border-slate-200'}`}>
              <CardHeader>
                <CardTitle className="text-xl flex items-center justify-between">
                  {t.name}
                  {t.highlight && <span className="text-xs px-2 py-1 rounded-full bg-brand-600 text-white">Most popular</span>}
                </CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <footer className="border-t border-slate-200">
        <div className="mx-auto max-w-6xl px-6 py-10 grid md:grid-cols-3 gap-6 text-sm text-slate-600">
          <div>
            <div className="h-8 w-8 rounded-xl bg-brand-600 text-white grid place-items-center font-bold">MSA</div>
